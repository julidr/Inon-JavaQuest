	...
	?>
			<div id="ContenedorPrincipal" name="ContenedorPrincipal" allign="center">
				...
				<div id="iniSU">
					...
					<center><p id="letIS">Iniciar Sesión Usuarios</p></center>
					<div id="is">
						<form role="form" id="inSU" name="inSU" method="Post" action="ISusuario.php">
							<?php
								if (isset($_POST["bingU"])) {
									$connection = mysqli_connect('localhost', 'root', '','inon');
									$emOus=$_POST["emOus"];
									$clave=$_POST["pwd"];
									$_SESSION['var']=0;
									$query="call iniciarSesion('".$emOus."','".$clave."');";
									$result=mysqli_query($connection, $query);
									$row=mysqli_fetch_assoc($result);
									if ($row['val']==1) {
										$_SESSION['usuarioNom']=$row['nombre']." ".$row['apellido'];
										$_SESSION['user']=$row["userCl"];
										$_SESSION['userid']=$row["idCl"];
										echo "<script> alert('Inicio de Sesión Exitoso') 
										document.location=('indexUser.php') </script>";
									}
									else{
										$_SESSION['var']=1;
									}
						  ...
						  <center><button type="submit" class="btn btn-default" id="bingU" name="bingU">Ingresar</button></center>
						</form>
						<?php
							if ($_SESSION['var']==1) {
